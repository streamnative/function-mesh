...
// +kubebuilder:webhook:path=/mutate-compute-functionmesh-io-v1alpha2-sink,mutating=true,failurePolicy=fail,groups=compute.functionmesh.io,resources=sinks,verbs=create;update,versions=v1alpha2,name=msink.kb.io,sideEffects=none,admissionReviewVersions={v1beta1,v1}
...
// +kubebuilder:webhook:verbs=create;update,path=/validate-compute-functionmesh-io-v1alpha2-sink,mutating=false,failurePolicy=fail,groups=compute.functionmesh.io,resources=sinks,versions=v1alpha2,name=vsink.kb.io,sideEffects=none,admissionReviewVersions={v1beta1,v1}
...
func (r *Sink) ValidateCreate() error {
	...
	if len(r.Name) > apispec.MaxNameLength {
		allErrs = append(allErrs, field.Invalid(field.NewPath("name"), r.Name,
			fmt.Sprintf("sink name must be no more than %d characters", apispec.MaxNameLength)))
	}
	...
	if r.Spec.Pod.VPA != nil {
		if r.Spec.MaxReplicas != nil {
			allErrs = append(allErrs,
				field.Invalid(field.NewPath("apispec").Child("pod").Child("vpa"), r.Spec.Pod.VPA,
