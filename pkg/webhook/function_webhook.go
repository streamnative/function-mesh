...
func (webhook *FunctionWebhook) Default(ctx context.Context, obj runtime.Object) error {
	...
	if len(r.Name) > maxNameLength {
		...
		// convert the long name to a short hash name with `function` as prefix
		r.Name = generateRandomName(r.Name, "function")

		functionlog.Info("function name is too long, generate a random name", "original name", r.Labels[OriginalNameLabel], "new name", r.Name)
	}
