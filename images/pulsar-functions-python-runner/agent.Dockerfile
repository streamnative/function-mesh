...
RUN mkdir -p /pulsar/bin/ \
    ...
    && chmod -R g=u /pulsar \
    && apk update && apk add --no-cache wget bash unzip \
...
COPY --chown=$UID:$GID ./streamnative-agent-engine /pulsar/streamnative-agent-engine/
...
RUN pip3 install litellm --user

RUN cd /pulsar/streamnative-agent-engine && \
    pip3 install -e . --user