...
manifests_file="${BASE_DIR}"/.ci/tests/integration/cases/mongodb-source-inferring-type/manifests.yaml
mongodb_file="${BASE_DIR}"/.ci/tests/integration/cases/mongodb-source-inferring-type/mongodb-dbz.yaml
...
verify_fm_result=$(ci::verify_function_mesh source-sample-inferring-type 2>&1)
if [ $? -ne 0 ]; then
