...
if [ $? -ne 0 ]; then
  ...
  kubectl delete -f "${BASE_DIR}"/.ci/tests/integration/cases/go-download-function/manifests.yaml > /dev/null 2>&1 || true
  exit 1
fi

verify_go_result=$(NAMESPACE=${PULSAR_NAMESPACE} CLUSTER=${PULSAR_RELEASE_NAME} ci::verify_download_go_function 2>&1)
if [ $? -eq 0 ]; then
